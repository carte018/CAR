...
public class IcmHttpClientFactory {
	...
	static {
		try {
			...
			client = HttpClients.custom().setDefaultRequestConfig(
					...
					).setConnectionManagerShared(true).setKeepAliveStrategy(new IcmConnectionKeepAliveStrategy()).setConnectionManager(pool).setSSLSocketFactory(factory).build();
			
			// Spin a thread to watch for idle connections in the pool manager
			
			IcmIdleConnectionMonitorThread watcher = new IcmIdleConnectionMonitorThread(pool);
			...
			IcmUtility.locLog("ERR1136", LogCriticality.error,e.getMessage());
		}
	}
	...
	public static CloseableHttpClient getHttpsClient() throws KeyManagementException, NoSuchAlgorithmException {
		...
		IcmUtility.locLog("ERR1136", LogCriticality.error,"Failed retrieving client from factory");
...
class IcmConnectionKeepAliveStrategy implements ConnectionKeepAliveStrategy {
...
class IcmIdleConnectionMonitorThread extends Thread {
 private final HttpClientConnectionManager connMgr;
 private volatile boolean shutdown;

 public IcmIdleConnectionMonitorThread(
