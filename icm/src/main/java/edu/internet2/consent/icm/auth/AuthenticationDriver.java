...
public class AuthenticationDriver {
	...
	public static String getAuthenticatedUser(HttpServletRequest request,HttpHeaders headers, IcmConfig config) {
		...
		try {
			...
			throw new RuntimeException(IcmUtility.locError(500,"ERR0029").getEntity().toString());
		}
		Class<BasicAuthHandler> validatorClass = null;
		try {
			validatorClass = (Class<BasicAuthHandler>) Class.forName(validator);
		} catch (Exception e) {
			throw new RuntimeException(IcmUtility.locError(500, "ERR0030", validator).getEntity().toString());
		}
		BasicAuthHandler v;
		try {
			v = (BasicAuthHandler) (validatorClass.newInstance());
		} catch (Exception e) {
			throw new RuntimeException(IcmUtility.locError(500,"ERR0031", validator).getEntity().toString());
		}
