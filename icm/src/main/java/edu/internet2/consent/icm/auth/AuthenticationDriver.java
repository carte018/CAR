...
public class AuthenticationDriver {
	...
	public static String getAuthenticatedUser(HttpServletRequest request,HttpHeaders headers, IcmConfig config) {
		String retUser = null;
		
		// Get the validator out of the configuration file
		String validator = null;
		try {
			validator = config.getProperty("icm.basicauth.validator.class", true);
		} catch (Exception e) {
			throw new RuntimeException(IcmUtility.locError(500,"ERR0029",LogCriticality.error).getEntity().toString());
		}
		Class<BasicAuthHandler> validatorClass = null;
		try {
			validatorClass = (Class<BasicAuthHandler>) Class.forName(validator);
		} catch (Exception e) {
			throw new RuntimeException(IcmUtility.locError(500, "ERR0030", LogCriticality.error, validator).getEntity().toString());
		}
		BasicAuthHandler v;
		try {
			v = (BasicAuthHandler) (validatorClass.newInstance());
		} catch (Exception e) {
			throw new RuntimeException(IcmUtility.locError(500,"ERR0031", LogCriticality.error, validator).getEntity().toString());
		}
