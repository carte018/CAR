...
public class CopsuUtility {
	...
	private static final Logger LOG=LoggerFactory.getLogger(CopsuUtility.class);
		...
		if (! registered) {
			try {
				...
				if (cfile.exists()) {
					...
					Class.forName(new Configuration().configure().getProperty("hibernate.connections.driver.class"));
				}
				registered = true;
			} catch (Exception e) {
				return null;
